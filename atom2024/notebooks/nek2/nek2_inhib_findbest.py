...
if __name__ == "__main__": 
    ...
    result1_best = find_best_models(train_x, train_y, test_x, test_y, 'Basic RF', {}, 
                                param_grid, 'Inhibition', 'NEK2inhib_GSCV_basicRF_best', verbose_val=2)
    best_rf = result1_best['best_model']
    best_params = best_rf.get_params()
    for param, value in best_params.items():
        print(f"{param}: {value}")
    pklfile = 'NEK2inhib_GSCV_basicRF_best.pkl'
    with open(pklfile, 'wb') as f:
        pickle.dump(best_rf, f)
    with open('NEK2inhib_GSCV_basicRF_best_results.pkl', 'wb') as f:
    ...
    result2_best = find_best_models(train_x, train_y, test_x, test_y, 'balanced class_weight', {'bootstrap':True, 'class_weight':'balanced'}, 
                                param_grid, 'Inhibition', 'NEK2inhib_gridsearch_classweightbal_best', verbose_val=2)
    best_rf2 = result2_best['best_model']
    best_params2 = best_rf2.get_params()
    for param, value in best_params2.items():
        print(f"{param}: {value}")
    pklfile = 'NEK2inhib_GSCV_RF_classweightbal_best.pkl'
    with open(pklfile, 'wb') as f:
        pickle.dump(best_rf2, f)
    with open('NEK2inhib_RF_classweightbal_best_results', 'wb') as f:
        pickle.dump(result2_best, f)

    result3_best = find_best_models(train_x, train_y, test_x, test_y, 'balanced RF', {'bootstrap':True}, 
                                param_grid, 'Inhibition', 'NEK2inhib_GSCV_BalRF_best', verbose_val=2)
    best_rf3 = result3_best['best_model']
    best_params3 = best_rf3.get_params()
    for param, value in best_params3.items():
        print(f"{param}: {value}")
    pklfile = 'NEK2inhib_GSCV_BalRF_best.pkl'
    with open(pklfile, 'wb') as f:
        pickle.dump(best_rf3, f)
    with open('NEK2inhib_BalRF_best_results.pkl', 'wb') as f:
        pickle.dump(result3_best, f)