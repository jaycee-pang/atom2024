...
def rf_results(model, x_input, y_labels, printout=False):
    ...
    if printout:
        print(
            f"accuracy: {acc:.3f}, precision: {precision:.3f}, recall: {recall:.3f}, specificity: {specificity:.3f}"
        )
    return {
        "prediction": pred,
        "accuracy": acc,
...
def rf_models(train_x, train_y, test_x, test_y, rf_type, parameters, printout=False):
    ...
    if printout:
        print(
            f"TRAIN: acc: {train_results['accuracy']:.3f}, precision: {train_results['precision']:.3f}, recall: {train_results['recall']:.3f}, spec: {train_results['specificity']:.3f}"
        )
        print(
            f"TEST: acc: {test_results['accuracy']:.3f}, precision: {test_results['precision']:.3f}, recall: {test_results['recall']:.3f}, spec: {test_results['specificity']:.3f}"
        )
